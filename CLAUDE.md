...
- **Deployment constraint**: Azure Container Apps runs a **single replica** (SQLite single-writer) with an **NFS** Azure Files share at `/pb_data`. NFS, not SMB — SMB lacks the POSIX byte-range locks SQLite WAL needs and crashlooped with `SQLITE_BUSY`. Don't introduce changes that assume horizontal scaling. `maxReplicas: 1` does **not** cover a rollout: ACA starts the incoming replica and waits for it to be *ready* before draining the outgoing one, and that ~40s overlap corrupted `auxiliary.db` five times (#35). `entrypoint.sh` fixes it by inverting the dependency — the incoming replica writes `/pb_data/.pb_handover`, the outgoing replica's watcher sees it and shuts itself down. **Never replace that with a blocking startup lock**: readiness gates the handover in both directions, so an incoming replica that blocks deadlocks the rollout (tried and reverted 2026-09-06, `b7642f75` / `2f46f48b`). The wait must stay **fail-open** for the same reason. `maxReplicas: 1` does **not** cover a rollout: ACA starts the incoming replica and waits for it to be *ready* before draining the outgoing one, so ~40s of two writers corrupts `auxiliary.db` on most deploys (#35). **Do not "fix" this with a blocking startup lock** — that was deployed on 2026-09-06 and deadlocks, because readiness gates the handover in both directions; see DEPLOY.md.
- **OTLP telemetry export**: `core/logger_otel.go` (fork-local) tees `app.Logger()` records to the parish SigNoz collector alongside the SQLite `_logs` sink, wired at the single `slog.New(...)` call in `core/base.go`. Gated on `OTEL_EXPORTER_OTLP_ENDPOINT` — **never** on `APPLICATIONINSIGHTS_CONNECTION_STRING`, since removing that setting must not silence OTLP. No endpoint = exact upstream behavior. Contract lives in `docs/observability/otlp-onboarding.md` in the STFoA-Church repo; setup in `DEPLOY.md`.
