...
- **Deployment constraint**: Azure Container Apps runs a **single replica** (SQLite single-writer) with an **NFS** Azure Files share at `/pb_data`. NFS, not SMB — SMB lacks the POSIX byte-range locks SQLite WAL needs and crashlooped with `SQLITE_BUSY`. Don't introduce changes that assume horizontal scaling. `maxReplicas: 1` does **not** cover a rollout: Container Apps starts the incoming replica before draining the outgoing one, and that ~40s overlap corrupted `auxiliary.db` four times (#35). `entrypoint.sh` holds an exclusive `flock` on `/pb_data/.pb_singlewriter.lock` on fd 9 for the life of the process to serialize it — don't remove it, and keep `PB_SINGLE_WRITER_TIMEOUT` (75s) below the startup probe's budget (`failureThreshold: 45` × 3s + 5s = 140s).
