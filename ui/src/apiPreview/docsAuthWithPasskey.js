export function docsAuthWithPasskey(collection) {
    const baseURL = app.utils.getApiExampleURL();

    const actionTabs = [
        { title: "Register passkey", content: register },
        { title: "Login with passkey", content: login },
        { title: "Manage credentials", content: manage },
    ];
    ...
    return t.div(
        {
            pbEvent: "apiPreviewAuthWithPasskey",
            className: "content",
        },
        // description
        t.p(
        ...
        t.p(
            null,
            "Both the registration and login flows are two-step: the client requests a challenge ",
            "(",
            t.code(null, "*-begin"),
            "), runs the browser WebAuthn ceremony, then sends the result back ",
            "(",
            t.code(null, "*-finish"),
            ").",
        ),
            ...
            tabs: [
                {
                    ...
                    value: `
                        ...
                        // ---
                        const begin = await fetch('${baseURL}/api/collections/${collection.name}/auth-with-webauthn/register-begin', {
                            method: 'POST',
                        ...
                        await fetch('${baseURL}/api/collections/${collection.name}/auth-with-webauthn/register-finish', {
                            method: 'POST',
                        ...
                        // ---
                        const lb = await fetch('${baseURL}/api/collections/${collection.name}/auth-with-webauthn/login-begin', {
                            method: 'POST',
                        ...
                        const auth = await fetch('${baseURL}/api/collections/${collection.name}/auth-with-webauthn/login-finish', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                ...
                },
                {
                    ...
                    value: `
                        ...
                        curl -X POST \\
                          ...
                          '${baseURL}/api/collections/${collection.name}/auth-with-webauthn/register-begin'
                        ...
                        curl -X POST \\
                          ...
                          '${baseURL}/api/collections/${collection.name}/auth-with-webauthn/register-finish'
                        ...
                        curl -X POST \\
                          ...
                          '${baseURL}/api/collections/${collection.name}/auth-with-webauthn/login-begin'
                        ...
                        curl -X POST \\
                          ...
                          '${baseURL}/api/collections/${collection.name}/auth-with-webauthn/login-finish'
                    `,
                },
            ],
        }),
...
function register(collection) {
    return [
        ...
        t.div(
            ...
            t.span({ className: "path" }, `/api/collections/${collection.name}/auth-with-webauthn/register-begin`),
        ),
        t.p(
            { className: "txt-hint" },
            "Requires the auth ",
            t.code(null, "Authorization"),
            " header of the record that will own the new passkey. Returns ",
        ...
        t.div(
            ...
            t.span({ className: "label method" }, "POST"),
            t.span({ className: "path" }, `/api/collections/${collection.name}/auth-with-webauthn/register-finish`),
        ),
        t.table(
            ...
            t.tbody(
                ...
                t.tr(
                    ...
                    t.td(null, "The session token returned by ", t.code(null, "register-begin"), "."),
                ),
                t.tr(
                    null,
                    t.td({ className: "min-width" }, "name"),
                    t.td({ className: "min-width" }, t.span({ className: "label" }, "String")),
                    t.td(null, "Optional human-readable label for the credential (e.g. \"My iPhone\")."),
                ),
                t.tr(
                    null,
...
function login(collection) {
    return [
        ...
        t.div(
            { className: "alert success api-preview-alert" },
            t.span({ className: "label method" }, "POST"),
            t.span({ className: "path" }, `/api/collections/${collection.name}/auth-with-webauthn/login-begin`),
        ),
        t.table(
            { className: "api-preview-table body-params" },
            t.thead(
            ...
            t.tbody(
                ...
                t.tr(
                    ...
                    t.td({ className: "min-width" }, t.span({ className: "label" }, "String")),
                    t.td(null, "Email or username of the record to sign in."),
                ),
            ),
        ),
        t.p(
            ...
            t.code(null, "options.publicKey"),
            " to ",
            t.code(null, "navigator.credentials.get()"),
            ".",
        ),
        t.div(
            { className: "alert success api-preview-alert m-t-sm" },
            t.span({ className: "label method" }, "POST"),
            t.span({ className: "path" }, `/api/collections/${collection.name}/auth-with-webauthn/login-finish`),
        ),
        t.p(
            { className: "txt-hint" },
            "Body: ",
...
function manage(collection) {
    return [
        ...
        t.div(
            ...
            t.span({ className: "path" }, `/api/collections/${collection.name}/auth-with-webauthn/credentials`),
        ),
        t.p({ className: "txt-hint" }, "Lists the authenticated record's registered passkeys."),
        t.div(
            { className: "alert success api-preview-alert m-t-sm" },
            t.span({ className: "label method" }, "PATCH"),
            t.span(
                { className: "path" },
                `/api/collections/${collection.name}/auth-with-webauthn/credentials/{credentialId}`,
            ),
        ),
        t.p(
            { className: "txt-hint" },
        ...
        t.p({ className: "txt-hint" }, "Removes a passkey from the authenticated record."),
        t.div(
            { className: "alert success api-preview-alert m-t-sm" },
            t.span({ className: "label method" }, "DELETE"),
            t.span(
                { className: "path" },
                `/api/collections/${collection.name}/auth-with-webauthn/credentials-by-record/{id}`,
            ),
        ),
        t.p(
            { className: "txt-hint" },
            "Superuser-only. Clears ",
            t.strong(null, "all"),
            " passkeys for the specified record (account recovery).",
        ),
    ];
}
