...
**Current:** synced to upstream **v0.39.8** (`cc4e8570`) on 2026-07-20.
WebAuthn library `go-webauthn/webauthn` **v0.17.4** (already latest — Step 3b
found no newer release); `modernc.org/sqlite` **v1.54.0** (upstream bumped from
v1.52.0; `modernc_versions_check.go` is upstream-owned and already matched
v1.54.0 / libc v1.74.1). Tests: full `go test ./...` green on
`feat/webauthn-passkey-support` (34/34 packages) and `deploy/azure` — the only
red was `apis/TestDefaultRateLimitMiddleware` on the deploy full-suite run, an
allowlisted timing flake that passed on isolated re-run. The Docker
